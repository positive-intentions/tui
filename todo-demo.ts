...
function selectNext() {
  ...
  if (selectedTodoId === null) {
    ...
    if (index < todos.length - 1) {
      selectedTodoId = todos[index + 1].id
      if (index + 1 >= scrollOffset + MAX_VISIBLE) {
        scrollOffset = Math.min(todos.length - MAX_VISIBLE, index + 1 - MAX_VISIBLE + 1)
      }
    }
  }
  renderTodos()
...
function handleKeyPress(key: KeyEvent) {
  if (confirmDialogOpen) {
    handleDialogKeyPress(key)
    return
  }

  if (editDialogOpen) {
    handleEditDialogKeyPress(key)
    return
  }

  const inputField = renderer?.root.findDescendantById("input-field") as Input | undefined
  const addBtn = renderer?.root.findDescendantById("add-btn") as Button | undefined

  if (inputField && addBtn && inputField.focused && !key.ctrl) {
    const hasText = inputField.value.trim().length > 0
    addBtn.setDisabled(!hasText)
  }
  ...
  switch (key.name) {
    ...
    case "d":
      if (selectedTodoId) {
        openConfirmDialog(selectedTodoId)
      }
      break
    case "return":
...
export async function run() {
  ...
  renderer!.setBackgroundColor(RGBA.fromInts(15, 23, 42, 255))
  ...
  mainRow.add(mainContainer)

  const titleBox = new BoxRenderable(renderer!, {
    id: "title-box",
    height: 3,
    backgroundColor: RGBA.fromInts(30, 58, 138, 255),
    border: true,
    borderStyle: "single",
    borderColor: RGBA.fromInts(59, 130, 246, 255),
    flexDirection: "row",
    alignItems: "center",
  ...
  const titleText = new TextRenderable(renderer!, {
    ...
    fg: RGBA.fromInts(56, 189, 248, 255),
  })
  titleBox.add(titleText)

  const drawerHint = new TextRenderable(renderer!, {
    id: "drawer-hint",
    content: "[?] Help",
    fg: RGBA.fromInts(250, 204, 21, 255),
    onMouseUp: (event: MouseEvent) => {
  ...
  mainContainer.add(inputBox)

  const inputLabel = new TextRenderable(renderer!, {
    id: "input-label",
    content: "Add: ",
    fg: RGBA.fromInts(148, 163, 184, 255),
  })
  inputBox.add(inputLabel)

  const inputField = new Input(renderer!, {
    id: "input-field",
    width: 30,
    placeholder: "Enter todo text...",
  ...
  const addBtn = new Button(renderer!, {
    id: "add-btn",
    label: "[Add]",
    width: 8,
    height: 3,
  ...
  const todoContainer = new BoxRenderable(renderer!, {
    id: "todo-container",
    flexGrow: 1,
    flexShrink: 1,
    flexDirection: "column",
    ...
    border: true,
    borderStyle: "single",
    borderColor: RGBA.fromInts(71, 85, 105, 255),
  })
  mainContainer.add(todoContainer)

  const todoList = new BoxRenderable(renderer!, {
    id: "todo-list",
    flexGrow: 1,
    flexShrink: 1,
    flexDirection: "column",
      ...
      if (event.scroll) {
        ...
        if (direction === "up") {
          ...
          if (selectedTodoId === null) {
            ...
            const index = todos.findIndex((t) => t.id === selectedTodoId)
            if (index < todos.length - 1) {
              selectedTodoId = todos[index + delta].id
              const newIndex = index + delta
              if (newIndex >= scrollOffset + MAX_VISIBLE) {
                scrollOffset = Math.min(todos.length - MAX_VISIBLE, newIndex - MAX_VISIBLE + 1)
              }
            }
          }
          renderTodos()
  ...
  todoContainer.add(todoList)

  const instructions = new TextRenderable(renderer!, {
    id: "instructions",
    content: "Selected: None | Press '?' for help",
    fg: RGBA.fromInts(100, 116, 139, 255),
    height: 2,
  })
  mainContainer.add(instructions)

  const drawer = new BoxRenderable(renderer!, {
    id: "drawer",
    width: 40,
    backgroundColor: RGBA.fromInts(30, 41, 59, 255),
    border: true,
    borderStyle: "double",
    borderColor: RGBA.fromInts(245, 158, 11, 255),
    flexDirection: "column",
    alignItems: "stretch",
    flexGrow: 0,
    flexShrink: 0,
  ...
  const drawerHeader = new BoxRenderable(renderer!, {
    id: "drawer-header",
    height: 3,
    backgroundColor: RGBA.fromInts(245, 158, 11, 255),
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    flexGrow: 0,
  ...
  const drawerTitle = new TextRenderable(renderer!, {
    id: "drawer-title",
    content: "INSTRUCTIONS",
    fg: RGBA.fromInts(15, 23, 42, 255),
  })
  drawerHeader.add(drawerTitle)

  const closeText = new TextRenderable(renderer!, {
    id: "close-text",
    content: "[?] Close",
    fg: RGBA.fromInts(15, 23, 42, 255),
    onMouseUp: (event: MouseEvent) => {
  ...
  drawer.add(instructionsBox)

  const instructionsText = new TextRenderable(renderer!, {
    id: "instructions-text",
    content: "",
    fg: RGBA.fromInts(226, 232, 240, 255),
  })
